document.addEventListener("DOMContentLoaded", function () {
    ...
    jsonData.projects.forEach(project => {
        ...
        card.classList.add("card", "card-bordered", "card-compact", "border-neutral", "hover:border-primary", "hover:shadow-md");
        ...
        technologyList.classList.add("card-actions", "justify-start");
        project.tools.forEach(tech => {
            const techBadge = document.createElement("div");
            techBadge.classList.add("badge", "badge-outline");
            techBadge.textContent = tech;
            technologyList.appendChild(techBadge);
        });

        const actionButtons = document.createElement("div");
        actionButtons.classList.add("card-actions", "justify-end");

        if (project.github_link) {
            const githubButton = createButton(project.github_link, "GitHub", "github");
            actionButtons.appendChild(githubButton);
        }

        if (project.playStore_link) {
            const playStoreButton = createButton(project.playStore_link, "Play Store", "play");
            actionButtons.appendChild(playStoreButton);
        }

        if (project.appStore_link) {
            const appStoreButton = createButton(project.appStore_link, "App Store", "apple");
            actionButtons.appendChild(appStoreButton);
        }

        if (project.websiteLink) {
            const websiteButton = createButton(project.websiteLink, "Website", "link");
            actionButtons.appendChild(websiteButton);
        }

        cardBody.appendChild(title);
        cardBody.appendChild(description);
        cardBody.appendChild(technologyList);
...
function createButton(link, text, icon) {
    ...
    path.setAttribute("d", getSvgPath(icon));
